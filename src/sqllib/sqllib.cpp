...
psql::Statement<psql::BindTypes<double, double, double, double>, psql::RetTypes<int64_t, double, double, int64_t, double, double, int64_t, double, double, double, double, double, int, int> > get_toshow_edges_select(boost::property_tree::ptree const& entries, psql::Database& db, bool named, std::string name)
{
    KeyValFilterTranslator tr("e.StartNodeID, ST_X(ST_StartPoint(e.Location::geometry)), ST_Y(ST_StartPoint(e.Location::geometry)), e.EndNodeID, ST_X(ST_EndPoint(e.Location::geometry)), ST_Y(ST_EndPoint(e.Location::geometry)), e.WayID",
    ...
    if (named)
        return psql::Statement<psql::BindTypes<double, double, double, double>, psql::RetTypes<int64_t, double, double, int64_t, double, double, int64_t, double, double, double, double, double, int, int> >(name, cr->create_sql(), db);
    else
        return psql::Statement<psql::BindTypes<double, double, double, double>, psql::RetTypes<int64_t, double, double, int64_t, double, double, int64_t, double, double, double, double, double, int, int> >(cr->create_sql(), db);
}
...
psql::Statement<psql::BindTypes<>, psql::RetTypes<int64_t, int64_t, double, double, std::string, std::string, int> > get_way_node_attributes_select(boost::property_tree::ptree const& entries, psql::Database& db, bool named, std::string name)
{
    KeyValFilterTranslator tr("wn.WayID, wn.NodeID, ST_X(n.Location::geometry), ST_Y(n.Location::geometry), COALESCE(na.Key, '') as Key, COALESCE(na.Value,'') as Value, wn.SequenceNo as SequenceNo",
    ...
    auto cr = SqlCreatorFactory::create(tr.translate(entries));
    if (named)
        return psql::Statement<psql::BindTypes<>, psql::RetTypes<int64_t, int64_t, double, double, std::string, std::string, int> >(name, cr->create_sql(), db);
    else
        return psql::Statement<psql::BindTypes<>, psql::RetTypes<int64_t, int64_t, double, double, std::string, std::string, int> >(cr->create_sql(), db);
}
