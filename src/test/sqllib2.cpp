...
BOOST_AUTO_TEST_CASE(kvtransast)
{
    sqllib::KeyValFilterTranslator tr("a.col1", "Test t, Abc a", "", "t", std::vector<std::string> {"asdf"}, "order by a");
    boost::property_tree::ptree entries;
    std::istringstream ss("<entries><entry><add><a>asdf</a></add><elements><el><key>aaa</key><value>*</value></el><el><value>bbbb</value></el></elements></entry><entry><add><a>asdfasdf</a></add><elements><el></el></elements></entry></entries>");
    boost::property_tree::xml_parser::read_xml(ss, entries);
    auto ret = tr.translate(entries);
    auto creat = sqllib::SqlCreatorFactory::create(ret);
    boost::regex r("\\s*\\(?\\s*\\(?select\\s+a.col1\\s*,\\s*asdf::asdf\\s+as\\s+a\\s+from\\s+Test t, Abc a\\s+where\\s+t.key\\s*=\\s*'aaa'\\s*\\)?\\s*intersect\\s*\\(?select\\s+a.col1\\s*,\\s*asdf::asdf\\s+as\\s+a\\s+from\\s+Test t, Abc a\\s+where\\s+t.value\\s*=\\s*'bbbb'\\s*\\)?\\s*\\)?\\s*union\\s*\\(?\\s*\\(?select\\s+a.col1\\s*,\\s*asdfasdf::asdf\\s+as\\s+a\\s+from\\s+Test t, Abc a\\s*\\)?\\s*\\)?\\s+order\\s+by\\s+a\\s*", boost::regex_constants::icase);
    BOOST_CHECK(boost::regex_match(creat->create_sql(), r));
}
