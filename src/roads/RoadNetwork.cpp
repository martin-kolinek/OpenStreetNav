...
void RoadNetwork::add_edge(RoadEdgeWithNodes const& e)
{
    ...
    edge_starts.insert(std::make_pair(e.start_node_id, start.get()));
    edge_ends.insert(std::make_pair(e.end_node_id, end.get()));

    start->neighbours.push_back(std::make_pair(e.cost, end.get()));

    for (auto it = edge_starts.lower_bound(e.end_node_id); it != edge_starts.upper_bound(e.end_node_id); ++it)
    {
        end->neighbours.push_back(std::make_pair(0, it->second));
    }
    for (auto it = edge_ends.lower_bound(e.start_node_id); it != edge_ends.upper_bound(e.start_node_id); ++it)
    {
